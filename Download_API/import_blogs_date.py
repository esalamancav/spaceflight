...
class Process:      
    ...
    def get_data(date):
        ...
        data_load = []
        get_article = Endpoints.Blog(session, date, limit)
        ...
        df_data = pd.DataFrame(data_load)
        df = df_data[["id", "title", "url", "image_url", "news_site", "summary", "published_at", "updated_at", "featured", "launches", "events"]]
        df["published_at"] = pd.to_datetime(df["published_at"])
        df["updated_at"] = pd.to_datetime(df["updated_at"])
...
if __name__ == '__main__':
    ...
    object_name = "raw/Blog_" + fecha_actual + ".csv" 

    #max_id = Process.get_id()
    df_api = Process.get_data(fecha_anterior)
    Process.load_s3(df_api, bucket_name, object_name )
    #df_api.to_csv(f"C:/Users/esala/Drive_Esteban/Esteban/Inetum/blogs.csv", index=False)
    #print(df_api.dtypes)
